...
def redraw_drift(did):
    # @login_required无法防止超权
    # 如uid为1的用户，发起的did是1
    # 但是却能修改uid为2的用户发起的did为2的请求
    with db.auto_commit():
        drift = Drift.query.filter_by(
                requester_id=current_user.id, id = did).first_or_404()
        # drift.pending已经被修改为枚举类型，否则无法接收PendingStatus.Redraw传递过来的值
...
def mailed_drift(did):
    with db.auto_commit():
        ...
        # 更新请求人的心愿清单中该书籍的状态，launched置为True表示该心愿已经达成
        # 该写法完成的功能与上面代码相同
